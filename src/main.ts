...
import { AppManager, WallMeasurement } from "./bim-components";
...
const wallMeasurement = components.get(WallMeasurement);
const raycaster = components.get(OBC.Raycasters).get(world);
// You can activate your casting however you like.
// Here, we are activating it once the user clicks.
window.addEventListener("click", () => {
  const result = raycaster.castRay();
  if (!result) return;
  const { object, instanceId, faceIndex } = result;

  // Not all casting results includes the instanceID and faceIndex.
  // Also, not all objects hit are FragmentMesh.
  // That means we must check those conditions passes as they are need
  // by our component.
  if (
    !(
  ...
  wallMeasurement.measure(object, faceIndex, instanceId);
});
